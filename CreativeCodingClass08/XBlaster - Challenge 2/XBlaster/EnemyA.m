...
static SKAction *scoreLabelAction = nil;
...
+ (void)loadSharedAssets
{
  ...
  dispatch_once(&onceToken, ^{
    ...
    moveBackAction = [SKAction sequence:@[[SKAction moveByX:0 y:20 duration:0.25]]];
      ...
      scoreLabelAction = [SKAction sequence:@[
                                [SKAction group:@[
                                        [SKAction scaleTo:1.0 duration:0],
...
- (id)initWithPosition:(CGPoint)position
{
  if (self = [super initWithPosition:position]) {
    ...
    _damageTakenPerShot = 5;
      ...
      _scoreLabel = [SKLabelNode labelNodeWithFontNamed:@"Thirteen Pixel Fonts"];
      _scoreLabel.name = @"scoreLabel";
      _scoreLabel.fontSize = 25.0f;
      _scoreLabel.fontColor = [SKColor colorWithRed:0.5 green:1.0 blue:1.0 alpha:1.0];
...
- (void)collidedWith:(SKPhysicsBody *)body contact:(SKPhysicsContact*)contact
{
  ...
  // If the enemies health is now below 0 then add the enemyDeath emitter to the scene and reset the enemies position to off screen
    ...
    [mainScene increaseScoreBy:_score];
        ...
        [mainScene.hudLayerNode addChild:_scoreLabel];
